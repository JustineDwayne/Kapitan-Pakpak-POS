...
namespace FastFoodPOS.Forms.AdminForms
{
    public partial class FormAddUser : UserControl
    {
        ...
        public FormAddUser()
        {
            ...
            validators.Add(new Validator(TextPassword2, LabelPassword, "Password", "required|min:8"));
            validators.Add(new Validator(TextConfirmPassword, LabelConfirmPassword, "Confirm Password", "required|min:8|compare"){ compare_control = TextPassword2});
        }
            ...
            try
            {
                if (validators.Count((validator) => validator.IsValid()) == validators.Count)
                {
                    var user = new User
                    {
                        ...
                        Password = Util.GetHashSHA256(TextPassword2.Text),
        ...
        private void VisibilityToggler1_Click(object sender, EventArgs e)
        {
            TextPassword2.UseSystemPasswordChar = !TextPassword2.UseSystemPasswordChar;

            if (TextPassword2.PasswordChar == '●')
            {
                TextPassword2.PasswordChar = '\0';
            }
        }

        private void VisibilityToggler2_Click(object sender, EventArgs e)
        {
